...
public class PayoutAnalyticsService {
  ...
  private static final Logger log =
      LoggerFactory.getLogger(PayoutAnalyticsService.class);
  private static final List<String> LIFECYCLE_EVENTS =
      List.of(PayoutAuditService.COMPLETED, PayoutAuditService.FAILED);
  ...
  public List<PayoutMethodBreakdownDTO> getMethodBreakdown(LocalDate startDate,
                                                           LocalDate endDate) {
    ...
    List<PayoutMethodBreakdownDTO> cached =
        readCachedBreakdown(startDate, endDate);
    if (cached != null) {
      return cached;
    }
    ...
    List<PayoutMethodBreakdownDTO> result =
        aggregateMethodBreakdown(start, end);
    writeCachedBreakdown(startDate, endDate, result);
    return result;
  }

  private List<PayoutMethodBreakdownDTO>
  readCachedBreakdown(LocalDate startDate, LocalDate endDate) {
  ...
  private void writeCachedBreakdown(LocalDate startDate, LocalDate endDate,
                                    List<PayoutMethodBreakdownDTO> result) {
    try {
      cacheService.putMethodBreakdown(startDate, endDate,
  ...
  private List<PayoutMethodBreakdownDTO>
  aggregateMethodBreakdown(LocalDateTime start, LocalDateTime end) {
    try {
      Aggregation aggregation = Aggregation.newAggregation(
          Aggregation.match(Criteria.where("action")
                                .in(LIFECYCLE_EVENTS)
                                .and("timestamp")
                                .gte(start)
                                .lte(end)
          ...
          Aggregation.group("payoutMethod")
              ...
              .sum(ConditionalOperators
                       .when(Criteria.where("completedCount").is(0))
                       .then(1)
                       .otherwise(0))
              .as("failureCount")
              .sum(ConditionalOperators
                       .when(Criteria.where("completedCount").is(1))
                       .thenValueOf("amount")
                       .otherwise(0))
              .as("totalAmount"),
      ...
      AggregationResults<Document> results = mongoTemplate.aggregate(
          aggregation, mongoTemplate.getCollectionName(PayoutAuditEvent.class),
          Document.class);
